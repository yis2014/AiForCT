...
learn_rate = 0.001
epochs = 20
train_dataset_path = 'D:/Desktop/CTAI-master/CTAI_model/data/'
...
def train():
    global res
    dataloaders = DataLoader(train_dataset, batch_size=8, shuffle=True, num_workers=0)
    ...
    plt.plot(res['epoch'], np.squeeze(res['loss']), label='Train cost')
    plt.ylabel('cost')
    plt.xlabel('epochs')
    plt.title("Model: train cost")
    plt.legend()

    plt.plot(res['epoch'], np.squeeze(res['dice']), label='Validation cost', color='#FF9966')
    plt.ylabel('loss')
    plt.xlabel('epochs')
